...
> **사전스코프 배선 (v0.8.5 — 질의유형 라우팅 + BM25 rerank + parent 적응. A/B 5라운드 실측).** 위키 내용 질의는 오케스트레이터가 **먼저 결정적 스코프**(0토큰)를 만든다:
> 1. **키워드 추출** — 질의에서 주제어 + **엔티티** + 동의어 3~6개(엔티티 포함이 recall의 진짜 레버 — 실측). 자연어 그대로 grep 금지.
> 2. **질의유형 분류(①라우팅)** → 스코프·전달방식을 분기한다(아래 표). 애매하면 **사실브리핑이 기본**(안전측).
> 3. `scope-expand.py`로 스코프 생성, synthesizer에 **경로/후보 목록만** 전달(내용 요약·나열 금지 — 슬림). 공통 지시: "**index 통독 금지**".
>
> | 질의유형 | 스코프 명령 | 전달(②parent 적응) | 근거 |
> |---|---|---|---|
> | **단일조회**("pending 몇 개", "X confidence") | `expand … --max 6` | seed 목록만(팩·full 불필요) | 최저비용 |
> | **사실브리핑·비교**("무엇 했나", "A vs B", 보고) | `expand … --max 20 --rerank 11` → `pack <상위 slug…>` | **claims 팩 1파일** | 실측 22~30k, 팩 1-read로 충분. rerank가 최대 레버(과하면 recall↓ — **11이 균형값**) |
> | **절차·how-to**("어떻게 도입/설정") | `expand … --max 8`(rerank 안 함) | **후보 목록만 → synthesizer가 full-read** | 절차는 세부 넓이 필요 → 팩=오버헤드. rerank는 needed 페이지 떨어뜨림(실측) |
>
> - **rerank는 사실브리핑 전용**(BM25 top-N). **11 고정**(8=과공격적 recall 0.4로 FirstPick류 실항목 누락, 15=팩 비대 무이득 — 실측 스윗스팟 11).
> - **절차질의는 스코핑 이득 없음**(baseline과 동률 ~38k, 본질적으로 여러 페이지 읽음) — rerank·팩 금지, full-read가 낭비 없는 최선.
> - recall은 `--max`가 아니라 **키워드 품질**로 올린다(실측). synthesizer가 부족분은 링크추적 full-read.
