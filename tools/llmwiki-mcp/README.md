...
- **On Windows this is weaker**: `ctime` there is the creation time, not the change time, so the `mtime`+`size`+`ino` triple carries the whole burden. If you rewrite files with external tools that restore timestamps, run with `LLMWIKI_CACHE=0`.
