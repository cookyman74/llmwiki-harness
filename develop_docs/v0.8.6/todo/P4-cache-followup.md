...
- [ ] P4-27+ (E2E 에서 발견) **Claude Code 가 structuredContent 를 모델에 넘기고, 그 안에 `text` 와 `rows`/`pages` 가 같은 정보를 이중으로 담아 모델 입력이 text 대비 2.06~4.24배**(pack 9,715B → 20,015B) — 팩의 토큰 절감 설계를 상쇄한다 ⏸ 2026-09-10 — 서버 계약(P2 outputSchema·contracts.ts·ajv 교차검증·tools-list 덤프)을 바꾸는 결정이라 사용자 판단 필요. 선택지: ① structuredContent 에서 `text` 제거(구조 필드만) ② structuredContent 자체를 빼고 text 만(outputSchema 제거) ③ 현행 유지. P3 라우팅 실측·클라이언트 호환(structuredContent 미지원 클라이언트)과 함께 재측정해야 한다
