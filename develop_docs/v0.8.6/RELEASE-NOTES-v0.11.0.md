# v0.11.0 — llmwiki MCP 서버 (npm `obsidian-llmwiki-mcp@0.1.0`)

> **상태(2026-09-11): npm `obsidian-llmwiki-mcp@0.1.0` 공개 완료 — P0~P4 전부 포함.** 레지스트리 shasum `15eb8eeb…` = main `5c72f90`. 이 문서가 `git tag v0.11.0`(2026-09-11) 의 릴리즈 노트다. 원안 이름 `llmwiki-mcp` 는 npm 유사도 정책으로 거부되어 바꿨고, 실행 명령은 그대로 `llmwiki-mcp` 다.
>
...
| 항목 | 결과 |
|---|---|
| 단위 테스트 | **45파일 602건** 통과 (공개 커밋 main `5c72f90`, P4 포함). 참고: P3 끝 `f098497` 은 39파일 545건 |
...
## 알려진 미충족 조건 (P3 외부리뷰 판정 — 사용자 결정으로 한계로 두고 공개)
...
- ~~`git tag v0.11.0`~~ — 2026-09-11 완료(주석 태그 + GitHub Release)
